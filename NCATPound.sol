...
contract NcatPound is Ownable {
    ...
    mapping (uint256 => Ncat) public ncatsInPound;
    ...
    function revealNcats() external {
        uint256[] memory randomIDs = _revealRandomNcats();
        for (uint256 i = 0; i < randomIDs.length; i++) { 
            Ncat memory ncat = ncatsInPound[randomIDs[i]];
            IERC721(ncat.nftContract).safeTransferFrom(address(this), msg.sender, uint256(ncat.ncatID));
        }
        for (uint256 i = 0; i < randomIDs.length; i++) { 
            uint256 count = ncatCount;
            if (randomIDs[i] < count) {
                ncatsInPound[randomIDs[i]] = ncatsInPound[count-1];
            }
            delete ncatsInPound[count-1];
            ncatCount--;
        }
    }
    ...
    // addNFTToPound allows for arbitrary NFT addition to the contract, assuming 
    // its ID fits within uint48. 
    function addNFTToPound(address nftContract, uint256 nftID) external onlyOwner() {
        require(nftContract != address(0), "NcatPound: addNFT zero addr");
        IERC721(nftContract).transferFrom(msg.sender, address(this), nftID);
        ncatsInPound[ncatCount] = Ncat(nftContract, uint64(nftID));
        ncatCount++;
    }
    ...
    function setNcatOwner(address newOwner) external onlyOwner() {
        INcat(NCAT).transferOwnership(newOwner);
    }
    ...
    function _revealRandomNcats() internal returns (uint256[] memory) {
        Commit memory commit = commits[msg.sender];
        require(commit.amount > 0 && commit.block != 0, "NcatPound: Need to commit");
        require(commit.block < uint64(block.number), "NcatPound: cannot reveal same block");
        require(commit.block + 255 >= uint64(block.number), "NcatPound: Revealed too late");
        ...
        uint256 _ncatCount = ncatCount;
        uint256 randomIndex = uint256(keccak256(abi.encodePacked(revealNonce, revealHash))) % _ncatCount;
        for (uint256 i = 0; i < commit.amount; i++) {
            randomIDs[i] = randomIndex;
            randomIndex = (randomIndex + 1) % _ncatCount;
        }
        revealNonce++;
        return randomIDs;
    }